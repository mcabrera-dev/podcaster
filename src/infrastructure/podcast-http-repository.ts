...
export class PodcastHttpRepository implements PodcastRepository {
  ...
  async findById(id: string): Promise<PodcastDetail> {
    const dtoToPodcastDetailTransform: DtoToPodcastDetailTransform =
      new DtoToPodcastDetailTransform();

    const response = await fetch(
      `${CORS_PROXY_RAW}url=${encodeURIComponent(
  ...
  async getEpisodes(feedUrl: string): Promise<PodcastEpisode[]> {
    ...
    const dtoToPodcastEpisodeTransform: DtoToPodcastEpisodeTransform =
      new DtoToPodcastEpisodeTransform();
